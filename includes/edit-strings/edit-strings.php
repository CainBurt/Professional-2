...
define( 'EDIT_STRINGS_DB_VERSION', 3 );
...
class Strings_List_Table extends WP_List_Table {
    ...
    public function get_columns() {
        $table_columns = array(		 
            ...
            'string_location' => __( 'Location', $this->plugin_text_domain ),
        );
...
function clear_all_strings() {
    global $wpdb;
    $table_name = $wpdb->prefix . 'crowd_edit_strings';
    $sql = "DROP TABLE IF EXISTS $table_name";
    $wpdb->query($sql);
}
add_action( 'wp_ajax_clear_all_strings', 'clear_all_strings' );
...
function edit_strings_admin_page_contents() {
    ?>
        ...
        <p>Example: <code>_x("Your text here", "Location", "crowd")</code>.<br/>(In <code>.twig</code> files, make sure this is within in the normal curly braces).</p>
        <button class='button delete-all'>DELETE ALL STRINGS</button>
        <p><small>DO THIS AT YOUR RISK. YOU WILL LOSE ALL STRING EDITS</small></p>
        <?php
...
function change_string( $translated_text, $text,$context,$domain ) {
    if ($domain == 'crowd') {
        global $wpdb;
        $table_name = $wpdb->prefix . 'crowd_edit_strings';
        $sql = "SELECT orig_string,replacement_string FROM $table_name WHERE `orig_string` = '$text' AND `string_location` = '$context';";
        $instance = $wpdb->get_row($sql, 'ARRAY_A');
        if (gettype($instance) == "array") {
...
add_filter( 'gettext_with_context', 'change_string', 100, 4 );